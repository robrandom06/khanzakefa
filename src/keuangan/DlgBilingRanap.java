...
public class DlgBilingRanap extends javax.swing.JDialog {
    ...
    private String biaya = "", tambahan = "", totals = "", norawatbayi = "", centangdokterranap = "", kd_pj = "",
            ...
            Host_to_Host_Bank_Jateng = "", Akun_BRI_API = "", Host_to_Host_Bank_Papua = "", Host_to_Host_Bank_Jabar = "", KodeBankJabar = "", PPN_Keluaran = "", Host_to_Host_Bank_Mandiri = "",kodeberkas="";
    ...
    private void BtnNotaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnNotaActionPerformed
        ...
        if (TNoRw.getText().trim().equals("") || TNoRM.getText().trim().equals("") || TPasien.getText().trim().equals("")) {
            ...
            try {
                ...
                i = 0;
                try {
                    biaya = (String) JOptionPane.showInputDialog(null, "Silahkan pilih nota yang mau dicetak!", "Nota", JOptionPane.QUESTION_MESSAGE, null, new Object[]{"Nota 1", "Nota 2", "Kwitansi", "Nota & Kwitansi", "Kwitansi Piutang", "Upload Nota Ke Berkas Digital"}, "Nota 1");
                    switch (biaya) {
                        case "Nota 1":
                            i = 1;
                            break;
                        ...
                        case "Kwitansi Piutang":
                            i = 5;
                            break;
                        case "Upload Nota Ke Berkas Digital":
                            i = 6;
                            break;
                    }
                } catch (Exception e) {
                ...
                if (i > 0) {
                    ...
                    if (i == 1) {
                        ...
                        String norawat = TNoRw.getText().replace("/", "");
                        String norm = TNoRM.getText();
                        String url = "http://" + koneksiDB.HOSTHYBRIDWEB() + ":"
                                + koneksiDB.PORTWEB() + "/"
                                + koneksiDB.HYBRIDWEB()
                                + "/billing/LaporanBilling2.php?petugas="
                                + akses.getkode().replaceAll(" ", "_") + "&tanggal="
                                + DTPTgl.getSelectedItem().toString().replaceAll(" ", "_")
                                + "&usere=" + koneksiDB.USERHYBRIDWEB()
                                + "&passwordte=" + koneksiDB.PASHYBRIDWEB();

                        convertToPDF(url,norawat,norm);
                    }
                    this.setCursor(Cursor.getDefaultCursor());
                }
            } catch (Exception ex) {
...
public void convertToPDF(String url,String norawat, String norm) {
    try {
        // Dapatkan lokasi aplikasi yang sedang berjalan
        String appPath = System.getProperty("user.dir");
        
        // Buat path ke folder tmpPdf
        String outputDir = Paths.get(appPath, "tmpPdf").toString();

        // Pastikan folder tmpPdf ada, jika tidak maka buat
        ...
        String outputPath = Paths.get(outputDir, "Billing"+norawat+"_"+norm+".pdf").toString();
        String FileName = "Billing"+norawat+"_"+norm;
        // Jalankan wkhtmltopdf untuk menyimpan file di tmpPdf
        String command = "wkhtmltopdf --print-media-type " + url + " " + outputPath;
    ...
    private void UploadPDF(String FileName, String docpath) {
        try {
            ...
            kodeberkas = Sequel.cariIsi("SELECT kode FROM master_berkas_digital WHERE nama LIKE '%Billing%'");
            if (Sequel.cariInteger("SELECT COUNT(no_rawat) AS jumlah FROM berkas_digital_perawatan WHERE lokasi_file='pages/upload/" + FileName + ".pdf'") > 0) {
                uploadSuccess = Sequel.mengedittf("berkas_digital_perawatan", "lokasi_file=?","no_rawat=?,kode=?, lokasi_file=?", 4, new String[]{
                    TNoRw.getText(),kodeberkas,"pages/upload/" + FileName + ".pdf", "pages/upload/" + FileName + ".pdf"
                });
            } else {
                uploadSuccess = Sequel.menyimpantf("berkas_digital_perawatan", "?,?,?", "No.Rawat", 3, new String[]{
                    TNoRw.getText(), kodeberkas, "pages/upload/" + FileName + ".pdf"
                });
            }
