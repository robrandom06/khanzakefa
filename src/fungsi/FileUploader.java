...
public class FileUploader {
    ...
    public static void UploadPDF1(String FileName, String docpath, String jenisBerkas, String noRawat, String noRM) {
    try {
        // Validasi input
        if (noRawat.trim().isEmpty() || noRM.trim().isEmpty()) {
            JOptionPane.showMessageDialog(null, "No. Rawat dan No. RM tidak boleh kosong!", "Peringatan", JOptionPane.WARNING_MESSAGE);
            return;
        }

        // Cek apakah noRawat valid di database
        if (Sequel.cariInteger("SELECT COUNT(*) FROM reg_periksa WHERE no_rawat='" + noRawat + "'") == 0) {
            JOptionPane.showMessageDialog(null, "No. Rawat tidak ditemukan dalam reg_periksa!", "Peringatan", JOptionPane.WARNING_MESSAGE);
            return;
        }
        ...
        boolean uploadSuccess;
        if (Sequel.cariInteger("SELECT COUNT(no_rawat) FROM berkas_digital_perawatan WHERE lokasi_file='pages/upload/" + FileName + ".pdf'") > 0) {
            uploadSuccess = Sequel.mengedittf("berkas_digital_perawatan", "lokasi_file=?", "no_rawat=?,kode=?, lokasi_file=?", 4, new String[]{
                noRawat, kodeberkas, "pages/upload/" + FileName + ".pdf", "pages/upload/" + FileName + ".pdf"
            });
        } else {
