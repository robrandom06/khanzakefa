...
public final class RMPenilaianAwalMedisIGD extends javax.swing.JDialog {
    ...
    private void initComponents() {
        ...
        TglAsuhan.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "29-08-2023 09:17:10" }));
...
//        Sequel.cariIsi("SELECT data_triase_igd.tekanan_darah, data_triase_igd.nadi, data_triase_igd.pernapasan, "
//                + "data_triase_igd.suhu, data_triase_igd.saturasi_o2 FROM data_triase_igd"
//                + " where data_triase_igd.no_rawat='" + TNoRw.getText() + "' ", TNoRM);
    ...
    public void autoLoadDataTTV() {
        if (!TNoRw.getText().trim().equals("")) { // Cek apakah ada No. Rawat
            if (Sequel.cariInteger("SELECT COUNT(no_rawat) FROM data_triase_igd WHERE no_rawat='" + TNoRw.getText() + "'") > 0) {
                TD.setText(Sequel.cariIsi("SELECT tekanan_darah FROM data_triase_igd WHERE no_rawat=?", TNoRw.getText()));
                Nadi.setText(Sequel.cariIsi("SELECT nadi FROM data_triase_igd WHERE no_rawat=?", TNoRw.getText()));
                RR.setText(Sequel.cariIsi("SELECT pernapasan FROM data_triase_igd WHERE no_rawat=?", TNoRw.getText()));
                SPO.setText(Sequel.cariIsi("SELECT saturasi_o2 FROM data_triase_igd WHERE no_rawat=?", TNoRw.getText()));
                Suhu.setText(Sequel.cariIsi("SELECT suhu FROM data_triase_igd WHERE no_rawat=?", TNoRw.getText()));
            }
        }
    }
