...
//            int totalKosong = 0; // Variabel untuk menghitung total nilai kosong
//            for (int i = 0; i < row; i++) {
//                for (int j = 0; j < tabMode.getColumnCount(); j++) {
//                    if (tabMode.getValueAt(i, j) == null || tabMode.getValueAt(i, j).toString().trim().isEmpty()) {
//                        System.out.println("Nilai kosong pada Row: " + i + ", Column: " + j);
//                        totalKosong++;
//                    }
//                }
//            }
//            System.out.println("Total nilai kosong: " + totalKosong);
            
            System.out.println("Jumlah Row = " + row);
            for (int i = 0; i < row; i++) {
                if (tabMode.getValueAt(i, 0).toString().trim().equals("")) {
                    tabMode.setValueAt("-", i, 0);
                }
                if (tabMode.getValueAt(i, 1).toString().trim().equals("")) {
                    tabMode.setValueAt("-", i, 1);
                }
                if (tabMode.getValueAt(i, 2).toString().trim().equals("")) {
                    tabMode.setValueAt("-", i, 2);
                }
                if (tabMode.getValueAt(i, 3).toString().trim().equals("")) {
                    tabMode.setValueAt("-", i, 3);
                }
                if (tabMode.getValueAt(i, 4).toString().trim().equals("")) {
                    tabMode.setValueAt("-", i, 4);
                }
                if (tabMode.getValueAt(i, 24).toString().trim().equals("")) {
                    tabMode.setValueAt("-", i, 24);
                }
                if (tabMode.getValueAt(i, 25).toString().trim().equals("")) {
                    tabMode.setValueAt("-", i, 25);
                }
                if (tabMode.getValueAt(i, 26).toString().trim().equals("")) {
                    tabMode.setValueAt("-", i, 26);
                }
                if (tabMode.getValueAt(i, 27).toString().trim().equals("")) {
                    tabMode.setValueAt("-", i, 27);
                }
                if (tabMode.getValueAt(i, 28).toString().trim().equals("")) {
                    tabMode.setValueAt("-", i, 28);
                }
                if (tabMode.getValueAt(i, 29).toString().trim().equals("")) {
                    tabMode.setValueAt("-", i, 29);
                }
        ...
        try {
            ...
            while (rs.next()) {
                ...
                tabMode.addRow(new Object[]{
                    ...
                    "Perawatan", "Operator 1", "Asisten Operator 1", "Instrumen", "Dokter Anak",
                    "Perawat Resusitas", "Dokter Anestesi", "Asisten Anestesi 1", "Asisten Anestesi 2", "Bidan 1", "Perawat Luar", "Onloop 1",
                    "Sewa OK/VK", "Alat", "Akomodasi", "N.M.S.", "Sarpras", "Dokter PJ Anak", "Dokter Umum", "Biaya Perawatan",
                    diagnosa_preop, diagnosa_postop, jaringan_dieksekusi, permintaan_pa, selesaioperasi, laporan_operasi
                });
                rs2 = koneksi.prepareStatement(
                        "select operasi.operator1, operasi.asisten_operator1,"
                        + "operasi.instrumen, operasi.dokter_anak, operasi.perawaat_resusitas, "
                        + "operasi.dokter_anestesi, operasi.asisten_anestesi,operasi.asisten_anestesi2, operasi.bidan, operasi.perawat_luar, "
                        + "operasi.omloop,operasi.dokter_pjanak,operasi.dokter_umum,"
                        + "operasi.kode_paket,paket_operasi.nm_perawatan, operasi.biayaoperator1, "
                        + "operasi.biayaasisten_operator1, operasi.biayainstrumen, "
                        + "operasi.biayadokter_anak, operasi.biayaperawaat_resusitas, operasi.biayadokter_anestesi, "
                        + "operasi.biayaasisten_anestesi,operasi.biayaasisten_anestesi2, operasi.biayabidan, operasi.biayaperawat_luar, operasi.biayaalat,"
                        ...
                        + "operasi.biayaasisten_operator1+operasi.biayainstrumen+"
                        + "operasi.biayadokter_anak+operasi.biayaperawaat_resusitas+operasi.biayadokter_anestesi+"
                        + "operasi.biayaasisten_anestesi+operasi.biayaasisten_anestesi2+operasi.biayabidan+"
                        + "operasi.biayaperawat_luar+operasi.biayaalat+operasi.biaya_dokter_pjanak+operasi.biaya_dokter_umum+"
                        + "operasi.biayasewaok+operasi.akomodasi+operasi.bagian_rs+operasi.biaya_omloop+operasi.biayasarpras) as total from operasi inner join paket_operasi "
